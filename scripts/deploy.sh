...
APP_FOLDER="~${USER}/${APP_FOLDER}"
SCP_URL="${USER}@${BUILD_URL}:${APP_FOLDER}"
SCP_SCRIPTS_URL="${USER}@${BUILD_URL}:${APP_FOLDER}/scripts/"
scp ${SSH_OPTS} artifacts.tar.gz "${SCP_URL}"
ssh ${SSH_OPTS} "${USER}@${BUILD_URL}" "mkdir -p "${APP_FOLDER}/scripts"" && scp ${SSH_OPTS} scripts/post_deploy.sh "${SCP_SCRIPTS_URL}"
